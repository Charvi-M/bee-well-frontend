...
// Send message - UPDATED to call handleBotResponse
...
// Handle bot response - UPDATED to properly manage typing indicator
async function handleBotResponse(userMessage) {
    console.log('Handling bot response for:', userMessage); // Debug log
    
    try {
        // Call backend API (typing indicator is already showing)
        const result = await callBackendAPI(userMessage);
        console.log('Adding bot message:', result); // Debug log
        
        const mdmessage=renderMarkdown(result.response); // Render markdown content
